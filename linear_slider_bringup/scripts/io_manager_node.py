...
class Button:
    def __init__(
        ...
        on_callback=None,
        off_callback=None,
        switch_on_callback=None,
        switch_off_callback=None
    ) -> None:
...
class IOManager(Node):
    def __init__(self):
        super().__init__(node_name="io_manager")
        ...
        self.joy_action_pub = self.create_publisher(Int16, "/joy_action", 1)
        ...
        self._sub_button_event = self.create_subscription(Joy, "/joy", self._sub_cb_button_event, 1, callback_group=self.service_cb)
        return

    def _sub_cb_button_event(self, msg: Joy):
        self.get_logger().warn(f"{msg.axes}")
    ...
    def send_joy_action(self, val):
        """We want to scale trigger floats to velocities. Slider max rpm is 1000. (need that translated)"""
        self.joy_action_pub.publish(Int16(data=val))
        return
